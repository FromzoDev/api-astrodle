...
import { AmateurOwnerService } from './amateur-owner.service.js';
...
export class AmateurOwnerController {
  ...
  @Auth(Role.Moderator)
  @Get()
  async getAmateurOwnersPaginated(@Query() queryDto: AmateurOwnerQueryDto, ): Promise<PaginatedApiResponse<AmateurOwner>> {
    const result = await this.amateurOwnerService.findPaginated(queryDto);

    return {
      code: HttpStatus.OK,
      message: result.items.length > 0 ? SuccessMessage.AMATEUR_OWNER_FETCHED_ALL : ErrorMessage.GLOBAL_NOT_FOUND_MESSAGE,
      data: result.items,
      pagination: {
        total: result.total,
        page: result.page,
  ...
  @Auth(Role.Moderator)
  @Get('/:id')
  async getAmateurOwnerById(@Param('id', ParseIntPipe) id: number): Promise<ApiResponse<AmateurOwner | null>> {
    return await this.amateurOwnerService.findOneById(id).then((amateurOwner) => ({
      code: HttpStatus.OK,
  ...
  @Auth(Role.Moderator)
  @Post()
  async createAmateurOwner(@Body() dto: createAmateurOwnerDTO): Promise<ApiResponse<AmateurOwner>> {
    return await this.amateurOwnerService.createAmateurOwner(dto).then((amateurOwner) => ({
  ...
  async updateAmateurOwner(
    ...
    return await this.amateurOwnerService.updateAmateurOwner(id, dto).then((amateurOwner) => ({
      code: HttpStatus.OK,
  ...
  @Auth(Role.Moderator)
  @Delete('/:id')
  async deleteAmateurOwner(@Param('id', ParseIntPipe) id: number): Promise<ApiResponse<null>> {
