...
export function pickRandomUnused(available: number[], alreadyUsed: number[]): number | null {
  const remaining = available.filter((i) => !alreadyUsed.includes(i));
  if (remaining.length === 0) return null;
  return remaining[Math.floor(Math.random() * remaining.length)];
}

export function pickRandomHint(alreadyUsedKeys: string[]): HintDefinition | null {
