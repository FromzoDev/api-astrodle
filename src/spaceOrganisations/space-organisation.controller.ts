...
export class SpaceOrganisationController {
  constructor(
    private readonly spaceOrganisationService: SpaceOrganisationService,
  ) {}

  @Get()
  async getSpaceOrganisationsPaginated(
    @Query() queryDto: SpaceOrganisationQueryDto,
  ): Promise<PaginatedApiResponse<SpaceOrganisation>> {
    const result = await this.spaceOrganisationService.findPaginated(queryDto);

    return {
      code: HttpStatus.OK,
      message:
        result.items.length > 0
          ? SuccessMessage.SPACE_ORGANISATION_FETCHED_ALL
  ...
  @Get('/:id')
  async getSpaceOrganisationById(
    ...
    schema: {
      ...
      properties: {
        ...
        countries: {
          type: 'string',
          description:
            'JSON-stringified array of countries, e.g. ["France","Germany"]',
        },
  ...
  async createSpaceOrganisation(
    @Body() createSpaceOrganisationDTO: createspaceOrganisationDTO,
    @UploadedFile() file?: MulterFile,
  ): Promise<ApiResponse<SpaceOrganisation>> {
    return await this.spaceOrganisationService
      .createSpaceOrganisation(createSpaceOrganisationDTO, file)
      .then((spaceOrganisation) => ({
  ...
  @Auth(Role.Moderator)
  @Delete('/:id')
  async deleteSpaceOrganisation(
    @Param('id', ParseIntPipe) id: number,
  ): Promise<ApiResponse<void>> {
    return await this.spaceOrganisationService
      .deleteSpaceOrganisation(id)
      .then((success) => ({
        code: HttpStatus.OK,
